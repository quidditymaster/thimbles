...
def approximate_normal_cdf(zscore):
    ...
    in_shape = zscore.shape
    zscore = np.atleast_1d(zscore)
...
class GaussianDensity(Density):
    ...
    def get_density_integral(self, index, coord):
        ...
        return approximate_normal_cdf(zscore)
...
class Box_Density(Density):
    def __init__(self, coord_centers):
        self.centers = coord_centers
        self.bins = centers_to_edges(coord_centers)
...
def get_resampling_matrix(input_centers, output_centers, pixel_density = None, 
    ...
    output_bins = centers_to_edges(output_centers)
    input_bins = centers_to_edges(input_centers)
